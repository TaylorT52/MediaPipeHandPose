...
def draw_landmarks_on_image(rgb_image, detection_result):
    ...
    annotated_image = np.copy(rgb_image)  # Use a copy of the input image
    ...
    for idx in range(len(hand_landmarks_list)):
        ...
        x_coordinates = [landmark.x * annotated_image.shape[1] for landmark in hand_landmarks.landmark]
        y_coordinates = [landmark.y * annotated_image.shape[0] for landmark in hand_landmarks.landmark]
        min_x, max_x = min(x_coordinates), max(x_coordinates)
        min_y, max_y = min(y_coordinates), max(y_coordinates)

        # Draw the bounding box.
        cv2.rectangle(annotated_image, (int(min_x), int(min_y)), (int(max_x), int(max_y)), (0, 255, 0), 2)

        # Get the top left corner of the detected hand's bounding box for handedness text.
        text_x = int(min_x)
        text_y = int(min_y) - MARGIN

        # Draw handedness (left or right hand) on the image.
        cv2.putText(annotated_image, f"{handedness[0].classification[0].label[0]}",
